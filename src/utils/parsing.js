...
export const processIphoneCsv = (result) => {
    ...
    const { aggregatedExpenses, expensesByPerson, expensesByMonth, totalIncome, totalExpense } = generateAggregatedData(cleanedData);
  
    // Return all needed data including totals
    return { cleanedData, aggregatedExpenses, expensesByPerson, expensesByMonth, totalIncome, totalExpense };
  };
  ...
  export const processAndroidCsv = (result) => {
    ...
    cleanedData.forEach((row) => {
      row["סטטוס"] = "ההעברה בוצעה"; // Status for Android CSVs
    });
  
    // Get aggregated data, including totals
    const { aggregatedExpenses, expensesByPerson, expensesByMonth, totalIncome, totalExpense } =
